...
5. **Prometheus exporter thread** (`prometheus-exporter`, only when `--prometheus <addr:port>` is set) — drives a current-thread tokio runtime hosting axum on the configured address, serving `/metrics`. Reads the same `SharedState` as the UI; Drop notifies a `tokio::sync::Notify` wired to axum's `with_graceful_shutdown` and joins.
